...
def load_guild_setup(guild_id):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT guild_token, guild_name, discord_role, server FROM guild_setup WHERE guild_id = ?", (guild_id,))
        row = cursor.fetchone()
        return {
            "guild_token": row[0] if row else None,
            "guild_name": row[1] if row else None,
            "discord_role": row[2] if row else None,
            "server": row[3] if row else SERVER
        }
...
def save_guild_setup(guild_id, data):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR REPLACE INTO guild_setup (guild_id, guild_token, guild_name, discord_role, server) VALUES (?, ?, ?, ?, ?)",
...
def load_user_data(guild_id):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT user_id, ign FROM user_data WHERE guild_id = ?", (guild_id,))
        return {row[0]: {"ign": row[1]} for row in cursor.fetchall()}

def save_user_data(guild_id, user_data):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM user_data WHERE guild_id = ?", (guild_id,))
        for user_id, data in user_data.items():
            cursor.execute(
                "INSERT INTO user_data (guild_id, user_id, ign) VALUES (?, ?, ?)",
                (guild_id, user_id, data["ign"])
            )
        conn.commit()
